...
docker service create --name $CONTROLER_NAME --constraint 'node.role == manager' --replicas=1 -p8091:8091 --network $NETWORK_NAME --mount type=bind,source=$1,destination=/tmp/config.xml $IMAGE_NAME sleep 3650000d
controlerId=''
